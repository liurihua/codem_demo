...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#f4f5f2] px-5 py-16 text-[#18221d]">
      <section className="mx-auto max-w-6xl">
        <nav className="mb-20 flex items-center justify-between">
          <div className="flex items-center gap-3 text-lg font-bold"><span className="grid size-10 place-items-center rounded-xl bg-[#173e2d] text-white">C</span>CodeM Studio</div>
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
          <div>
            <p className="mb-5 text-sm font-bold uppercase tracking-[.22em] text-[#d46442]">Customer collaboration</p>
        ...
        <div className="mt-16 grid gap-5 md:grid-cols-2">
          {projects.map((project) => (
            <Link key={project.id} href={`/p/${project.slug}`} className="group rounded-[2rem] border border-[#dce0dc] bg-white p-8 shadow-[0_18px_60px_rgba(26,45,34,.06)] transition hover:-translate-y-1">
              ...
              <div className="mt-8 flex items-center justify-between border-t border-[#edf0ed] pt-5 text-sm"><span>{project.customer}</span><span className="font-bold text-[#d46442] group-hover:translate-x-1">查看项目 →</span></div>
            </Link>
          ))}
        </div>
      </section>
