...
export default async function ProjectPage({ params, searchParams }: PageProps<"/p/[slug]">) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f5ef] text-[#1e2621]">
      ...
      <section className="mx-auto max-w-6xl px-5 py-14">
        <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
          <div>
            <p className="text-sm font-bold text-[#d46442]">{project.customer}</p>
            ...
            <div className="mt-10 rounded-3xl bg-[#173e2d] p-8 text-white">
              <div className="flex justify-between text-sm"><span>项目整体进度</span><strong>{project.progress}%</strong></div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-[#e88b65]" style={{ width: `${project.progress}%` }} /></div>
          ...
          <aside id="feedback" className="h-fit rounded-3xl border border-[#dfded6] bg-white p-7 shadow-[0_20px_60px_rgba(33,43,36,.08)] lg:sticky lg:top-8">
            <p className="text-sm font-bold text-[#d46442]">SUBMIT FEEDBACK</p><h2 className="mt-2 text-2xl font-semibold">告诉我们需要怎么改</h2><p className="mt-2 text-sm leading-6 text-[#6b746e]">提交后，团队会在后台接收并更新处理状态。</p>
            ...
            <form action={action} className="mt-6 space-y-4"><label className="block text-sm font-semibold">姓名<input name="author" required maxLength={40} className="mt-2 w-full rounded-xl border border-[#d9ddd9] px-4 py-3 outline-none focus:border-[#173e2d]" /></label><label className="block text-sm font-semibold">联系方式（选填）<input name="contact" maxLength={100} placeholder="邮箱或手机号" className="mt-2 w-full rounded-xl border border-[#d9ddd9] px-4 py-3 outline-none focus:border-[#173e2d]" /></label><label className="block text-sm font-semibold">修改需求<textarea name="content" required maxLength={2000} rows={6} placeholder="请描述希望调整的内容、效果或优先级" className="mt-2 w-full resize-none rounded-xl border border-[#d9ddd9] px-4 py-3 outline-none focus:border-[#173e2d]" /></label><button className="w-full rounded-xl bg-[#173e2d] px-5 py-3.5 font-bold text-white hover:bg-[#22543c]">提交需求</button></form>
          </aside>
        </div>
      </section>
    </main>
